"""
automated_analysis_with_retry.py — combined pipeline (automated analysis (2).py
+ extract_failed_gp.py) that VERIFIES the G' summary and RETRIES missing batches.

Flow:
  1. run the batch pipeline  -> builds Gp_summary.csv for every sample
  2. scan Gp_summary.csv for missing G'/G'' values (recovering cheaply from
     cached output xlsx files where possible)
  3. if anything is still missing, RE-RUN the batch (it only reprocesses the
     missing batches) to find the missing data
  4. scan again — if values are still missing, STOP with an error (exit 1).

Usage:
    python3 automated_analysis_with_retry.py

Reports every scan to failed_gp_report.csv in the batch base dir.
"""
...
# ── NEW: MIN_SIZE used by the completeness scan (same valid-file filter as the pipeline) ──
MIN_SIZE = 93000


# ══════════════════════════════════════════════════════════════
#  BATCH ORCHESTRATOR (refactored from __main__ batch branch)
# ══════════════════════════════════════════════════════════════
def run_batch_pipeline(base_dir=BASE_DIR_FOR_BATCH):
    """Process every sample in base_dir, building the grouped G' summary.

    Skips samples whose batch xlsx outputs already exist and only re-runs
    the missing batches (spawns THIS script in single-run mode).
    """
...
# ══════════════════════════════════════════════════════════════
#  SINGLE-SAMPLE PIPELINE (refactored from __main__ single branch)
# ══════════════════════════════════════════════════════════════
def run_single_sample_pipeline():
    """Run the full Excel -> Prism -> MATLAB pipeline for one sample.

    Spawned as a subprocess by run_batch_pipeline via PRISM_SAMPLE_DIR /
    PRISM_BATCH_START / PRISM_BATCH_END env vars, or called directly.
    """
    global FOLDER_PATH   # restore module-global semantics (was module-level in the original __main__)
...
#################### PART 2 — completeness scan (extract_failed_gp.py) ####################

def valid_pulling_files(sample_dir):
    files = sorted(sample_dir.glob("*.txt"), key=natural_sort_key)
    return [f for f in files if "pulling" in f.name and os.path.getsize(f) > MIN_SIZE]


def sample_found_count(condition, sample, summary_path):
    """How many G' 0.1Hz values does the sample currently have in the grid?"""
    if not summary_path.exists():
        return 0
    blocks = read_gp_summary_grid(str(summary_path))
    block = blocks.get("G' 0.1Hz")
    if block is None:
        return 0
    col = f"{condition} {sample}"
    if col not in block.columns:
        return 0
    return int(block[col].notna().sum())


def migrate_legacy_to_grid(legacy_path, grid_path, condition):
    """Copy EVERY sample row from an old wide Gp_values.csv into the grid.

    Runs BEFORE any recovery/upsert so no old value can be lost (an upsert
    removes the legacy file once the new one exists).
    """
    if not legacy_path.exists():
        return 0
    df = pd.read_csv(legacy_path)
    prefixes = ["gp01_", "gp1_", "gp10_", "gdp01_", "gdp1_", "gdp10_",
                "td01_", "td1_", "td10_"]

    def pull_num(col):
        m = re.search(r"pull(\d+)", col)
        if m:
            return int(m.group(1))          # new format: "gp01_1_pull2" -> pull 2
        # legacy format: "gp01_1" means G' 0.1Hz bead 1 (the FIRST value).
        # Legacy has no separate pull number, so the bead number doubles as
        # the pull stand-in -> labels read 1_1, 2_2, ..., 10_10.
    ...
    for _, row in df.iterrows():
        ...
        # If this sample has real output xlsx files, SKIP the legacy row — it
        # will be recovered from the xlsx with TRUE pull numbers instead of
        # the 1_1-style stand-in (legacy columns carry no pull info).
        real_outputs = sorted(legacy_path.parent.glob(f"{condition}_{sample}_bead*.xlsx"))
        if real_outputs:
            print(f"  Skipping legacy row for {condition}/{sample} — "
                  f"{len(real_outputs)} xlsx output(s) carry real pulls")
            continue
        rd = row.to_dict()
        pulls = None
        values_by_prefix = {}
        for prefix in prefixes:
            cols = sorted([c for c in rd if c.startswith(prefix)], key=pull_num)
            pairs = [(pull_num(c), rd[c]) for c in cols if pd.notna(rd[c])]
            if pulls is None and pairs:
                pulls = [p for p, _ in pairs]
            values_by_prefix[prefix] = [v for _, v in pairs]
        if not pulls:
            continue
        # renumber as first_num + i so the grid pull rows are correct
        first_num = min(pulls)
        values_by_prefix = {p: values_by_prefix[p] for p in prefixes}
        upsert_sample_in_gp_summary(str(grid_path), condition, sample,
                                        first_num, values_by_prefix)
        migrated += 1
    print(f"  Migrated {migrated} sample(s) from legacy {legacy_path.name}")
    return migrated


def format_pull_ranges(nums):
    """[13, 14, 15, 16, 21, 22] -> '13-16, 21-22'"""
    if not nums:
        return ""
    nums = sorted(nums)
    parts = []
...
def sample_found_pulls(condition, sample, summary_path):
    """Set of pull numbers currently in the grid for this sample."""
    if not summary_path.exists():
        return set()
    blocks = read_gp_summary_grid(str(summary_path))
    block = blocks.get("G' 0.1Hz")
    ...
    if col not in block.columns:
        return set()
    pulls = set()
    for lbl in block[col].dropna().index:
        try:
            pulls.add(int(str(lbl).split("_")[-1]))
        except Exception:
            continue
    return pulls


def sample_has_tan_delta(condition, sample, summary_path):
    """True when the grid already has at least one non-zero tanδ value
    for this sample (any frequency)."""
    if not summary_path.exists():
        return False
    try:
        blocks = read_gp_summary_grid(str(summary_path))
        ...
        return False
    col = f"{condition} {sample}"
    for label in ("tanδ 0.1Hz", "tanδ 1Hz", "tanδ 10Hz"):
        block = blocks.get(label)
        if block is None or col not in block.columns:
            continue
...
def scan_and_report(base_dir):
    """Classify every sample under base_dir against Gp_summary.csv.

    Recovers values from cached xlsx outputs where possible. Writes
    failed_gp_report.csv and returns the list of report dicts.
    """
    base = Path(base_dir).expanduser()
    if not base.exists():
        cp.err(f"ERROR: base dir does not exist: {base}")
        return []

    print(f"\nVerifying G' summary completeness under {base}\n")
    conditions = discover_samples_for_batch(base)
    total = sum(len(s) for s in conditions.values())
    if total == 0:
        cp.err("No sample folders found under this base dir.")
        return []
    cp.info(f"{total} samples across {len(conditions)} conditions\n")
    ...
    for condition, samples in sorted(conditions.items()):
        for sample, sample_dir in sorted(samples.items()):
            cond_dir = sample_dir.parent
            summary_path = cond_dir / GP_SUMMARY_FILENAME
            if cond_dir not in legacy_done:
                migrate_legacy_to_grid(cond_dir / "Gp_values.csv", summary_path, condition)
                legacy_done.add(cond_dir)
            valid = valid_pulling_files(sample_dir)
            expected = (len(valid) // BEAD_COUNT) * BEAD_COUNT
            found = sample_found_count(condition, sample, summary_path)

            marker = sample_dir / "missing_beads.txt"
            xlsx_outputs = sorted(cond_dir.glob(f"{condition}_{sample}_bead*.xlsx"))
            xlsx_outputs = [p for p in xlsx_outputs if not p.name.startswith("~$")]

            # ── classify ──
            if len(valid) == 0:
                status = "NO PULLING FILES"
                note = "no valid pulling .txt files found"
            elif expected == 0:
                status = "TOO FEW PULLS"
                note = f"{len(valid)} valid pulls, need >= {BEAD_COUNT}"
            elif marker.exists():
                status = "SKIPPED (missing beads)"
                note = f"marker {marker.name} present"
            elif found > expected:
                status = "EXTRA (stale?)"
                note = (f"{found} pulls in summary but only {expected} valid now — "
                        f"an old batch is probably still in Gp_summary.csv")
            elif found >= expected:
                status = "OK"
                note = f"{found}/{expected} pulls in summary"
            elif xlsx_outputs:
                status = "PARTIAL -> recovering"
                note = f"{found}/{expected} pulls in summary, re-reading {len(xlsx_outputs)} xlsx"
                for xp in xlsx_outputs:
                    m = re.search(r"bead(\d+)-(\d+)\.xlsx$", xp.name)
                    if not m:
                        continue
                    start, end = int(m.group(1)), int(m.group(2))
                    try:
                        append_gp_and_gdp_summary(sample_dir, xp, start, end - start + 1)
                        recovered_any = True
                    except Exception as exc:
                        print(f"    ! recovery failed for {xp.name}: {exc}")
                found = sample_found_count(condition, sample, summary_path)
                status = "RECOVERED" if found >= expected else "STILL PARTIAL"
                note = f"{found}/{expected} pulls now"
            else:
                status = "NOT PROCESSED"
                note = "no output xlsx, no summary entry (pipeline never finished)"

            # ── tanδ backfill ──
            if (xlsx_outputs
                    and status not in ("NO PULLING FILES", "TOO FEW PULLS",
                                       "SKIPPED (missing beads)")
                    and not sample_has_tan_delta(condition, sample, summary_path)):
                added = 0
                for xp in xlsx_outputs:
                    m = re.search(r"bead(\d+)-(\d+)\.xlsx$", xp.name)
                    if not m:
                    ...
                    try:
                        ...
                        print(f"    ! tanδ recovery failed for {xp.name}: {exc}")
                if added:
                    recovered_any = True
                    if sample_has_tan_delta(condition, sample, summary_path):
                        status = "TANδ ADDED"
                        note = f"tanδ extracted from {added} xlsx output(s)"
                    else:
                        status = "TANδ MISSING"
                        note = "workbooks yielded no tanδ (total_result will compute G'\'\'/G')"

            # ── which pulls/batches are missing (for debugging) ──
            missing_desc = ""
            if marker.exists():
                try:
                    lines = [l.strip() for l in marker.read_text(encoding="utf-8").splitlines() if l.strip()]
                    missing_desc = "missing: " + ", ".join(lines[1:]) if len(lines) > 1 else f"marker: {lines[0]}"
                except Exception:
                    missing_desc = "marker present"
            elif status not in ("OK", "RECOVERED", "EXTRA (stale?)") and valid and expected > 0:
                try:
                    first_pull = get_number(valid[0].name)
                except Exception:
                    first_pull = None
                if first_pull:
                    expected_pulls = set(range(first_pull, first_pull + expected))
                    found_pulls = sample_found_pulls(condition, sample, summary_path)
                    missing = sorted(expected_pulls - found_pulls)
                    if missing:
                        missing_desc = "missing " + format_pull_ranges(missing)
            elif status == "TOO FEW PULLS" and valid:
                have = []
                for v in valid:
                    try:
                        n = get_number(v.name)
                        ...
                        pass
                if have:
                    missing_desc = "have pulls " + format_pull_ranges(have)
            if missing_desc:
                note = f"{note} [{missing_desc}]"

            line = f"  [{status:22s}] {condition}/{sample}  ({note})"
            if status in ("OK", "RECOVERED"):
                cp.ok(line)
            elif status in ("STILL PARTIAL", "TOO FEW PULLS", "EXTRA (stale?)"):
                cp.warn(line)
            else:
                cp.err(line)
            report.append({
                "condition": condition, "sample": sample,
                "expected_pulls": expected, "found_pulls": found,
                "status": status, "note": note,
                "missing_batches": missing_desc,
            })

    out = base / "failed_gp_report.csv"
...
def samples_needing_retry(report):
    """Samples whose G' values are still not complete (need a batch re-run)."""
    return [r for r in report if r["status"] not in ("OK", "RECOVERED", "EXTRA (stale?)")]


#################### PART 3 — combined orchestrator with retry ####################

def main():
    """Run the batch, verify the G' summary, retry missing batches, stop if still missing."""
    base = Path(BASE_DIR_FOR_BATCH).expanduser()
    if not base.exists():
        cp.err(f"ERROR: batch base dir does not exist: {base}")
        sys.exit(1)

    # 1) build the G' summary for every sample
    cp.section("\nPASS 1 — running the batch pipeline to build the G' summary")
    run_batch_pipeline(base)

    # 2) check for missing data in the summary
    cp.section("\nCHECK 1 — scanning Gp_summary.csv for missing values")
    report = scan_and_report(base)
    missing = samples_needing_retry(report)

    if not missing:
        cp.ok("\nAll samples complete after the initial batch. Nothing to retry.")
        sys.exit(0)

    cp.warn(f"\n{len(missing)} sample(s) missing data after the initial batch — retrying...")
    for r in missing:
        print(f"  {r['status']}: {r['condition']}/{r['sample']} — {r['note']}")

    # 3) retry: re-run the batch — it only reprocesses the missing batches
    cp.section("\nPASS 2 — retrying the batch to recover missing data")
    run_batch_pipeline(base)

    # 4) re-check — if still missing, STOP
    cp.section("\nCHECK 2 — re-scanning after retry")
    report2 = scan_and_report(base)
    still_missing = samples_needing_retry(report2)

    if still_missing:
        cp.err("\nSTOPPING — the following samples are STILL missing data after retry:")
        for r in still_missing:
            print(f"  {r['status']}: {r['condition']}/{r['sample']} — {r['note']}")
        cp.err(f"See {base}/failed_gp_report.csv for the full report.")
        sys.exit(1)

    cp.ok("\nAll samples recovered after retry. G' summary is complete.")
    sys.exit(0)


if __name__ == "__main__":
    # spawn of the batch orchestrator -> run the single-sample pipeline only
    if os.environ.get("PRISM_SINGLE_RUN"):
